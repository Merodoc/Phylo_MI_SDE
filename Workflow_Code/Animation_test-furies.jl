...
xlims!(0, 5)
ylims!(-5, 5)
xlabel!("Branch Length (relative)")
ylabel!("Difference in Trait")
title!("Brownian Bridge Between Two Species")
anim = @animate for x = 1:300
    for i in 1:10
        y = collect(eachcol(bridgeanim)[i])
        if x > 250
            push!(p, i, t[250], y[250])
        else
        push!(p, i, t[x], y[x])
...
gif(anim, "BMMidasAnim-Forward.gif")
