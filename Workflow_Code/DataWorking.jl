...
function Phy_Bridge_SimSDE2(start_dir, end_dir, tree, max_iter, init_samples, diffusion = "OUMean", vars = (1., 0.1))
    #Read MI files from start_dir
    Files = readdir(start_dir)
    try
        mkdir(end_dir)
        ...
        # We should now have a data frame that has a sample per species from every variable
            ...
            for trait in names(sampled_df)
                ...
                if trait == "Species"
                    ...
                    for j in 1:max_iter              
                        results = Leaf_PruneSDE2(mars_tree, sampled_df, trait, vars, diffusion)
                        if j == 1
                            trait_data[!, :Species] = results[!, :Species]
                        end
                        raw_data = select(results, Not([:Species]))
...
dir3 = "/home/theaeg/Documents/PhD/Phylo_MI_SDE/Workflow_Code/MI_Data/"
#Files2 = readdir(dir2)
