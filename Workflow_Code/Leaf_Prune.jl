...
function Phylo_BridgeSDE(start, fin, fin_time, anc, dt, samples, β = 0.0, σ = 1.0)
    ...
    for n in N
        ...
        B = sample(0:dt:fin_time, WienerBridge(fin_time,fin), start)
        #
        X = solve(EulerMaruyama(), 0.1, B, OrnsteinUhlenbeck(β,σ))
...
function Phylo_BridgeOUMean(start, fin, fin_time, anc, dt, samples, α, μ, σ)
    #start = start value
    #fin = final value
    #fin_time = total time
    #anc = time point of the ancestor node
    ...
    for n in N
        # Solve an OU bridge beteen start and fin
        # Wiener Bridge to guide the proposed Diffusion Bridge
        B = sample(0:dt:fin_time, WienerBridge(fin_time,fin), start)
        # Proposed Diffusion Bridge Process
        Prop = BridgeProp(OUMean(α, μ, σ), 0:dt:fin_time, (fin_time, fin), start)
        #Generate a Guided Proposal 
        X = solve(EulerMaruyama(), B, Prop)
        idx = findall(x -> x == anc, X.tt)
        val = X.yy[idx][1]
        push!(Xhat, val)
    end
...
function Phylo_BridgeCIR(start, fin, fin_time, anc, dt, samples, α, μ, σ)
    #start = start value
    #fin = final value
    #fin_time = total time
    #anc = time point of the ancestor node
    ...
    for n in N
        ...
        B = sample(0:dt:fin_time, WienerBridge(fin_time,fin), start)
        # Proposed Diffusion Bridge Process
        Prop = BridgeProp(CIR(α, μ, σ), 0:dt:fin_time, (fin_time, fin), start)
        #Generate a Guided Proposal 
        X = solve(EulerMaruyama(), B, Prop)
        idx = findall(x -> x == anc, X.tt)
        val = X.yy[idx][1]
...
function Phylo_BridgeWF(start, fin, fin_time, anc, dt, samples, α, μ, σ)
    #start = start value
    #fin = final value
    #fin_time = total time
    #anc = time point of the ancestor node
    ...
    for n in N
        ...
        B = sample(0:dt:fin_time, WienerBridge(fin_time,fin), start)
        # Proposed Diffusion Bridge Process
        Prop = BridgeProp(WF(α, μ, σ), 0:dt:fin_time, (fin_time, fin), start)
        #Generate a Guided Proposal 
        X = solve(EulerMaruyama(), B, Prop)
        idx = findall(x -> x == anc, X.tt)
        val = X.yy[idx][1]
...
function Leaf_PruneSDE(tree, df, variable, species = "Species", dt = 0.01, samples = 100)
    #Currently this iterates through all the nodes in the tree from leaves to root and returns the list
root = first(nodenamefilter(isroot, tree))
data = DataFrame()
...
for leaf in leaves
    ...
    #optimizing this depends on what inputs the bridge needs
            ...
            time = round(time, digits = 2)
            anc_time = round(bridgelen[1], digits = 2)
            bridgesim = Phylo_BridgeSDE(val1, val2, time, anc_time, dt, samples)
            trait_sample = sample(bridgesim)
            Xhat = mean(bridgesim)
            Var = var(bridgesim)
...
function Leaf_PruneSDE2(tree, df, variable, vars, SDE, species = "Species", dt = 0.01, samples = 100)
    #Currently this iterates through all the nodes in the tree from leaves to root and returns the list
root = first(nodenamefilter(isroot, tree))
data = DataFrame()
#sp_names = Vector{Int64}()
